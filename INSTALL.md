...
| `CurseForge packs without a published server pack are not installable yet` | Attendu. Un manifeste CurseForge liste des identifiants, pas des URL : chaque mod demande un appel supplémentaire et une redirection. Choisis une version dont l'éditeur fournit un server pack. Modrinth, lui, fonctionne. |
| Un pack Modrinth s'installe, mais des mods manquent dans `mods/` — le log panel montre `could not queue a manifest file` avec `reached its limit of 3 simultaneous remote file downloads` | Corrigé : Wings n'autorise que 3 téléchargements en fond simultanés par serveur, et un manifeste de plusieurs centaines de mods les mettait tous en file d'un coup. `ThrottledPull` réessaie désormais chaque mod refusé pour cette seule raison (backoff court, borné) ; un vrai échec (404, permissions) n'est lui jamais réessayé. Si le message persiste malgré une version à jour, regarde le log **complet** : `queued`/`failed` dans la note d'installation donne le compte exact. |
| Un pack Modrinth s'installe mais le serveur ne démarre pas tout de suite | Les mods sont mis en file chez Wings et se téléchargent en arrière-plan. Laisse-leur un moment et regarde l'onglet Files : `mods/` doit se remplir. |
